...
class ViewController: UIViewController {
    ...
    func setupWebViews() {
        ...
        // 3. Prezi (Sağ Üst Küçük Kutu - 200x120 boyutunda)
        preziView = WKWebView(frame: .zero)
        preziView.translatesAutoresizingMaskIntoConstraints = false
        preziView.layer.borderWidth = 2
        preziView.layer.borderColor = UIColor.orange.cgColor // Turuncu çerçeve (yüklendiğini gör diye)
    ...
    func setupControls() {
        ...
        let b2 = UIButton(type: .system); b2.setTitle("OYNAT/TAM", for: .normal); b2.addTarget(self, action: #selector(goFullScreen), for: .touchUpInside)
    ...
    @objc func goFullScreen() { 
        // Prezi'nin içindeki hem giriş hem tam ekran butonlarını tetikle
        preziView.evaluateJavaScript("document.querySelector('.prezi-player-icon-play')?.click();")
        preziView.evaluateJavaScript("document.querySelector('.present-button')?.click();")
    }
...
// MARK: - WKWebView Extensions & Delegates (Çerezler İçin)
